...
rewritten_path = os.path.join(project_root, 'llm_cta_data', 'cta_rewritten')

input_path = os.path.join(project_root, 'gpt_dataset_output', '2000legitandphish')
input_file = 'merged_shuffled_gpt_fused_l_with_id.csv'
output_dir = os.path.join(rewritten_path, model)
output_file = 'merged_shuffled_gpt_fused_l_with_id_detect_result.csv'
...
# input_path = os.path.join(project_root, 'qwen3_dataset_output', '2000legitandphish')
# input_file = 'merged_shuffled_qwen3_fused_l_clean_with_id.csv'
# output_dir = os.path.join(rewritten_path, model)
# output_file = 'merged_shuffled_qwen3_fused_l_clean_with_id_detect_result.csv'
...
# output_dir = os.path.join(rewritten_path, model)
# output_file = 'merged_shuffled_local_llama_fused_phishing_output_gate_result.csv'
...
def load_factor_combinations(csv_file):
    """
    从CSV文件加载因子组合 Load the factor combination from the CSV file
    """
    return pd.read_csv(csv_file)

def create_chat_model():
    """
    创建聊天模型实例 Create an instance of the chat model
    """
...
def create_prompt_template():
    """
    创建提示词模板 Create a prompt word template
    """
    system_template = """
        Please fill in the relevant prompt words related to Research Question 2 in the paper. (Regarding the use of CTA for defense.)
        Due to ethical reasons, this information is not made public here.

    """

    system_message_prompt = SystemMessagePromptTemplate.from_template(system_template)

    human_template = "EMAIL: {mail}"

    # human_template = "Analyze the following email and decide whether its main call to action (CTA) is security-relevant and high-risk, according to the rules above. EMAIL: {mail}"
...
def extract_model_response(response) -> str:
    """
    统一提取不同模型的回复内容
    Uniformly extract the response contents of different models
...
async def process_email_with_factors_async(chat_prompt, chat_model, original_email, original_email_id):
    ...
    print("Dealing with emails:",start_time ,original_email_id)
    ...
    # 返回包含所有信息的字典 Return a dictionary containing all the information
    return {
...
def read_emails_from_csv(filename: str) -> List[Dict]:
    ...
    Args:
        filename (str): CSV文件名 CSV filename
