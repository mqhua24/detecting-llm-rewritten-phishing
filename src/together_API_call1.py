...
INPUT_CSV = "../fusion_dataset_output/phishing_fused_1000.csv"
OUTPUT_CSV = f"../together_llama_dataset_output/together_llama_fused_phishing_output{type}.csv"
ERROR_CSV = f"../together_llama_dataset_output/together_llama_fused_phishing_errors{type}.csv"
...
def call_api_single(content: str):
    ...
    human_message_prompt = HumanMessagePromptTemplate.from_template(f"{prompt_prefix_selected}{content}")
    #prompt = ChatPromptTemplate.from_template(f"{prompt_prefix_selected}{content}")
...
def batch_process_concurrent(input_csv: str, output_csv: str, error_csv: str,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="Processing rows"):
            ...
            if hasattr(rewritten, 'content') and rewritten.content:
                rewritten = rewritten.content
            #print("相同：", rewritten == orig_text)
            # 按原始索引位置存储结果
